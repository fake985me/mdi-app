    ...
    <div class="mb-6 border-b border-gray-200">
      <nav class="-mb-px flex space-x-8">
        <router-link 
          ...
          :class="[$route.name === 'PageBuilderMain' || $route.name === 'PageBuilderEdit' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300']"
        >
          Page Builder
        </router-link>
        <router-link 
          to="/page-builder/all-pages" 
          class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm"
          :class="[$route.name === 'PageBuilderAllPages' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300']"
        >
          All Pages
        </router-link>
        <router-link 
          ...
          class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm"
          :class="[$route.name === 'PageBuilderTemplates' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300']"
        >
          Page Templates
        </router-link>
        <router-link 
          to="/page-builder/navigation" 
          class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm"
          :class="[$route.name === 'PageBuilderNavigation' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300']"
        >
...
const exportPage = async () => {
  if (!route.params.id) {
    alert('Please save the page first before exporting.')
    return
  }

  try {
    // Show a loading indicator or confirmation
    if (!confirm(`Are you sure you want to export this page to the public views? This will make it accessible at /${currentPage.value.slug}`)) {
      return
    }
    
    // Call the export API endpoint
    const response = await fetch(`/api/pages/${route.params.id}/export`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        theme: 'default' // Could be dynamic based on user selection
      })
    })

    if (response.ok) {
      const result = await response.json()
      alert(`Page exported successfully! Accessible at: ${result.public_url}`)
    } else {
