...
import { Plus, Edit2, Trash2, Box } from 'lucide-vue-next'
...
const filteredSubcategories = computed(() => {
  ...
  return warehouseStore.subcategories.filter(sub => sub.category_id === selectedCategory.value)
})

const availableSubcategories = computed(() => {
  if (!productForm.category_id) return []
  return warehouseStore.subcategories.filter(sub => sub.category_id === productForm.category_id)
})
...
const filteredProducts = computed(() => {
  ...
  if (selectedSubcategory.value) {
    filtered = filtered.filter(p => p.subcategory_id === selectedSubcategory.value)
  }
