              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div v-for="product in paginatedProducts" :key="product.id"
                  ...
                  <RouterLink :to="{
                    ...
                    <div class="w-full h-48 bg-gray-200 flex items-center justify-center rounded-t-lg p-4">
                      <span class="text-gray-500">Product: {{ product.title }}</span>
                    </div>
                  </RouterLink>
                  <div class="px-4 py-3">
...
// State aktif
const activeCategory = ref('XGSPON')
...
const currentSubCategories = computed(() => {
  const filtered = productsData.filter((p) => p.category === activeCategory.value)
